            ...
            public void onClick(View v) {
                ...
                db.child("Users").child(product.getUserUid()).child("Delivery").child(product.getDeliveryNumber()).child(product.getProduct_code()).addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        ...
                        // Обновление цены
                        Integer productPrice = Integer.parseInt(product.getProduct_price());  // Получаем цену товара (в копейках)
                        Integer newPrice = (productPrice / currentQuantity) * (currentQuantity + 1);  // Пересчитываем цену с новым количеством
                        ProductList.get(position).setProduct_price(String.valueOf(newPrice));
                        holder.product_price.setText(String.format("%d BYN", newPrice));

                        db.child("Users").child(product.getUserUid()).child("Delivery").child(product.getDeliveryNumber()).child(product.getProduct_code()).child("product_price").setValue(String.valueOf(newPrice));
                    }
                ...
                db.child("Users").child(product.getUserUid()).child("Delivery").child(product.getDeliveryNumber()).child(product.getProduct_code()).addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        if(!holder.quantity.getText().toString().equals("1")) {
                            ...
                            Integer newPrice = (productPrice / currentQuantity) * (currentQuantity - 1);  // Пересчитываем цену с новым количеством
                            ProductList.get(position).setProduct_price(String.valueOf(newPrice));
                            holder.product_price.setText(String.format("%d BYN", newPrice));  // Обновляем текст с новой ценой (в BYN)
